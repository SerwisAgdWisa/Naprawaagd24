...
try:
    ...
    AUTO_TECH_FACTS_DB = {}  # если файл ещё не сгенерирован — просто пусто, не падаем

# =========================================================
# ЗАГРУЗКА .ENV И НАСТРОЙКА ЛОГИРОВАНИЯ
# =========================================================
load_dotenv()
...
# =========================================================
# ИСКЛЮЧЕНИЯ И GRACEFUL SHUTDOWN
# =========================================================
...
class GracefulShutdown:
    """Управление безопасным прерыванием программы (SIGINT/SIGTERM)."""
    def __init__(self):
...
# =========================================================
# БАЗА ДАННЫХ, ЛИМИТЕР И МОНИТОРИНГ
# =========================================================
...
class Metrics:
    ...
    def estimate_cost(self) -> float:
        # $0.0001 за 1k токенов dla Llama-3.1-8b
...
def safe_write_file(path: Path, content: str):
    """Атомарная запись с восстановлением из .bak бэкапа при сбое."""
    ...
    try:
        ...
        if backup_path.exists():
            backup_path.unlink() # Удаляем бэкап при успешной записи
    except Exception as e:
        if backup_path.exists():
            backup_path.rename(path) # Откат
        raise SEOGeneratorError(f"Сбой записи в {path}: {e}") from e

def split_into_paragraphs(text: str) -> list[str]:
    """Умное разбиение текста от AI на ровные 3 абзаца."""
    if '\n\n' in text:
    ...
    # Запасной алгоритм: группировка предложений
...
def extract_streets_from_soup(soup: BeautifulSoup) -> list[str]:
    ...
    # 1. Считываем geo.placename из meta
...
EXISTING_CONTENT_CLASSES = [
    'local-info', MARKER_CLASS,       # старые AI-вставки (как раньше)
    'symptoms-table', 'faq-item',     # реальный контент, уже написанный
    'expertise-box', 'service-card',  # руками/шаблоном на странице
]


def extract_existing_seo_text(soup: BeautifulSoup) -> str:
    """
    Собирает текст ИЗ ВСЕГО реального контента страницы (таблица симптомов,
    FAQ, блок экспертизы, карточки услуг), а не только из старых AI-вставок.
    Это нужно, чтобы промпт "знал", что уже написано на странице, и не
    повторял то же самое, а дополнял недостающее.
    """
    extracted = []

    # 1. Именованные блоки контента (таблицы, FAQ и т.д.)
    for tag in soup.find_all(['section', 'div', 'table'], class_=EXISTING_CONTENT_CLASSES):
        text = tag.get_text(separator=' ', strip=True)
        if text:
            extracted.append(text)

    # 2. Если ничего из вышеперечисленного не нашлось (страница ещё
    #    без ручного контента) — берём общий текст <main>, если есть,
    #    как запасной вариант, чтобы промпт не остался пустым.
    if not extracted:
    ...
    # Ограничиваем длину — иначе промпт разрастётся и будет дороже/медленнее
    MAX_CHARS = 2000
...
def update_html_dom(soup: BeautifulSoup, generated_text: str, appliance: str, city: str, brand: str) -> BeautifulSoup:
    ...
    # 3. Безопасное создание фрагмента без рекурсивного импорта
    new_section = BeautifulSoup(new_block_html, 'html.parser').find('section')

    # 4. Точная вставка в DOM (<main> -> <footer> -> <body>)
    main_tag = soup.find('main')
    if main_tag:
...
def main():
    ...
    if not GROQ_API_KEY:
        logging.critical("Переменная GROQ_API_KEY не задана в .env или окружении!")
        sys.exit(1)
...
Napisz 3 zwięzłe akapity techniczne dla serwisu w mieście {city_slug.capitalize()}."""
        ...
        if args.apply and cached_text and not args.dry-run:
            try:
